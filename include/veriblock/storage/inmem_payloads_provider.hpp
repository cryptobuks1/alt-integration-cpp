...
struct InmemPayloadsProvider : public PayloadsProvider {
  ...
  std::vector<ATV> getATVs(const std::vector<ATV::id_t>& ids) override {
    return getPayload<ATV>(ids);
  }
  std::vector<VTB> getVTBs(const std::vector<VTB::id_t>& ids) override {
    return getPayload<VTB>(ids);
  }
  std::vector<VbkBlock> getVBKs(
      const std::vector<VbkBlock::id_t>& ids) override {
    return getPayload<VbkBlock>(ids);
  }
